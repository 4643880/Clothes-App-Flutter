...
class AdminAllOrdersScreen extends StatelessWidget {
  AdminAllOrdersScreen({Key? key}) : super(key: key);

  final currentOnlineUser = Get.put(CurrentUserState());

  Future<List<Order>> getAllOrdersList() async {
    List<Order> allOrrdersList = [];
    try {
      String url = API.receivedOrdersAdmin;
      ...
      if (response.statusCode == 200) {
        ...
        if (decodedResponseBody["success"] == true) {
          ...
          (decodedResponseBody["allOrdersData"] as List)
              .forEach((eachOrder) {
            allOrrdersList.add(Order.fromJson(eachOrder));
          });
        } else {
